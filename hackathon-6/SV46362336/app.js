...
const addBranchInput = document.getElementById('addBranchInput')
...
function loadBranches() {
    ...
    if (storedBranches) {
        const parseBranches  = JSON.parse(storedBranches)
        branches = parseBranches.map(data => new Branch(data.branchName))
    }
}
...
ticketForm.addEventListener('submit', (event) => {
        ...
        if (percentageAmount >= 50) {
            alert('Pago mínimo del 50%, iniciar reparación')
        }
...
function editTicket(id) {
    ...
    if (ticketToEdit) {
        ...
        branchSelect.value = ticketToEdit.branch.branchName
...
function deleteTicket(id) {
    if (confirm('¿Estás seguro que deseas borrar este ticket?')) {
...
addBranchBtn.addEventListener('click', () => {
    const newBranchName = addBranchInput.value.trim()
    if (newBranchName && !branches.some(b => b.branchName === newBranchName)) {
        branches.push(new Branch(newBranchName))
        saveBranches();
        renderBranchOptions()
        addBranchInput.value = ''
    } else if (newBranchName) {
        alert('La sucursal ya existe.')
    } else {
